...
#Chimp-Galaxy Concept
```{r}

#This project analysing and transoforms and visualises chimps (centrality scores) into galaxies in a plot and eventually a gif. Individual characteristics such as age, dominance, flow betweenness, and centrality measures were used. I used the dataset of the respiratory outbreak the Ngogo chimpanzees (2020) in this project, manipulating the centrality scores to actualise the dancing chimps-turned-galaxies concept. 
...
p <- p |>
  mutate(
    ...
    eigen_scaled = rescale(eigenvector.centrality, to = c(2, 8)),
    strength_scaled = rescale(strength.centrality, to = c(0.5, 1.5)),
    x = runif(n(), min = -1, max = 1),  # Generate random x values 
    y = runif(n(), min = -1, max = 1)   # Generate random y values 
  )

# Assigning constellation colors
